...
export const updateParticipantInDatabase = async (table, id, updateData) => {
  try {
    const response = await fetch(`https://vercel.com/o-thos-projects/feastly-server2/${table}/${id}`, {
      method: 'PUT',
      headers: {
...
export const updateCheckbox = async (table, id, restaurant, data) => {
  try {
    const response = await fetch(`https://vercel.com/o-thos-projects/feastly-server2/${table}/${id}/${restaurant}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
